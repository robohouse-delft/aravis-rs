...
impl<O: IsA<DomCharacterData>> DomCharacterDataExt for O {
    fn get_data(&self) -> Option<GString> {
        unsafe {
            from_glib_none(aravis_sys::arv_dom_character_data_get_data(self.as_ref().to_glib_none().0))
        }
    }

    fn set_data(&self, value: &str) {
        unsafe {
            aravis_sys::arv_dom_character_data_set_data(self.as_ref().to_glib_none().0, value.to_glib_none().0);
        }
    }
}
