...
impl FakeDevice {
    pub fn new(serial_number: &str) -> FakeDevice {
        ...
        unsafe {
            Device::from_glib_full(aravis_sys::arv_fake_device_new(serial_number.to_glib_none().0)).unsafe_cast()
        }
    }
}
...
impl<O: IsA<FakeDevice>> FakeDeviceExt for O {
    fn get_fake_camera(&self) -> Option<FakeCamera> {
        unsafe {
            from_glib_none(aravis_sys::arv_fake_device_get_fake_camera(self.as_ref().to_glib_none().0))
        }
    }
}
